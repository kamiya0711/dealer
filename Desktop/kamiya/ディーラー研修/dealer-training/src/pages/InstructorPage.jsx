...
export default function InstructorPage() {
  ...
  const getStatusColor = (p, game) => {
    if (!p?.[game.id]?.quizDone) return 'bg-slate-300'
    const score = p[game.id].bestScore / p[game.id].quizTotal
    return score >= 0.8 ? 'bg-emerald-500' : 'bg-amber-400'
  }

  return (
    <div className="px-4 py-6 max-w-lg mx-auto">
      <h2 className="text-2xl font-bold text-slate-800 mb-1">インストラクター</h2>
      <p className="text-slate-500 text-sm mb-6">研修生の進捗一覧</p>

      {userList.length === 0 ? (
        <div className="bg-white rounded-2xl p-8 text-center border border-slate-200 shadow-sm">
          <div className="text-4xl mb-3">👤</div>
          <p className="text-slate-500">まだ研修生が登録されていません</p>
          <p className="text-slate-400 text-sm mt-2">研修生がログインすると自動的に表示されます</p>
        </div>
      ) : (
                      ...
                      {availableGames.map(g => {
                        ...
                        return (
                          <div key={g.id} className="flex items-center gap-3">
                            ...
                            <span className={`text-xs px-2 py-0.5 rounded-full ${gp.studyDone ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                              📖 {gp.studyDone ? '読了' : '未読'}
                            </span>
